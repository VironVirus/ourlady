...
function buildOpeningPrayer({
  ...
  return [
    ...
    saintName
      ? `Through the prayers of ${saintName}, keep the students of this chaplaincy faithful, joyful, and close to your altar. Amen.`
      : "Keep the students of this chaplaincy faithful, joyful, and close to your altar. Amen."
  ];
}
...
function buildCyonReflection({
  ...
  return {
    eyebrow: `Student Reflection for ${weekday}`,
    title: focus,
    body: saintName
      ? `${celebration} invites students to listen to Christ with the same faith and generosity shown by ${saintName}. Let today's word shape how you speak, how you serve, and how you carry Catholic compassion into the classroom, clinic, ward, and chapel.`
      : `${celebration} calls the students of the chaplaincy to stay close to Christ today. Receive the word prayerfully, carry it into your study, your friendships, and your future service in healthcare, and let your witness be calm, sincere, and Catholic.`,
    challenge
  };
}
...
export default async function DailyReadingsPage() {
  ...
  return (
    <div className="page">
      <PageIntro
        eyebrow="Chaplaincy Daily Readings"
        title={celebration}
        description="Follow today’s readings, saint of the day, and a simple prayer path for students."
          ...
          <article className="story-article__main">
            ...
            {liturgicalDay?.readings ? (
              <div className="today-panel__readings">
                ...
                <div className="today-panel__reading-list">
                  {standardReadings.map((reading) => (
                    ...
                    >
                      ...
                      {reading.text ? (
                        <div className="today-panel__reading-body">
                          {reading.text
                            .split(/\n+/)
                            .map((line) => line.trim())
                            .filter(Boolean)
                            .map((line, index) => (
                              <p key={`${reading.citation}-${index}`}>{line}</p>
                            ))}
                        </div>
                      ) : (
                        <p className="today-panel__reading-note">
                          Passage text is not available right now. The citation is still correct for today.
                        </p>
                      )}
                    </div>
                  ))}
                ...
                {gospelAcclamation ? (
                  <div className="daily-devotion-card daily-devotion-card--acclamation">
                    ...
                    <p className="daily-devotion-card__acclamation-text">
                      {gospelAcclamation.text || gospelAcclamation.citation}
                    </p>
                    <small>
                ...
                <p className="today-panel__reading-note">
                  Citations follow {liturgicalDay.readings.sourceLabel}. Passage text is shown in{" "}
                  {liturgicalDay.readings.translationLabel} from {liturgicalDay.readings.textSourceLabel}.
                </p>
              </div>
            ) : (
              <div className="panel panel--soft-stack">
                ...
                <p>The reading references are not available right now. Please check again shortly.</p>
              </div>
            )}
          </article>
